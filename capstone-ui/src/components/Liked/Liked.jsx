          ...
          match.scoreInfo.liked ? (
            <div key={index} className="card">
                ...
                >
                  <div>
                    <img
                      ...
                      src={match.userInfo.profile_photo}
                    />
                  </div>
                  {match.userInfo.ig_media
                    ...
                    : getMediaArray(match.userInfo.media, []).map(
                        (photo, index) => (
                          <div>
                            <img
                              key={index}
              ...
              <p className="card-text">
                <b>Match Score: {(match.scoreInfo.score * 100).toFixed(1)}%</b>
              </p>
              <p className="card-text">
                University: {match.userInfo.university}
              </p>
              <p className="card-text">
                Graduation Year: {match.userInfo.grad_year}
              </p>
              <p className="card-text">Major: {match.userInfo.major?.name}</p>
              <p className="card-text">Hometown: {match.userInfo.hometown}</p>
              <p className="card-text">
                {match.userInfo.tags.length > 0
                  ? `Tags: ${match.userInfo.tags}`
                  : ""}
              </p>
              {Array.isArray(match.interestsInfo.movies) &&
              match.interestsInfo.movies.length > 0 ? (
                <div>
                  <p className="card-text match-interests">Movies: </p>
                  {match.interestsInfo.movies.map((movie, index) => (
                    <p
                      key={index}
                      className="card-text match-interests match-movies"
                    >
                      {movie.title}
                    </p>
                  ))}
              ...
              <p></p>
              {Array.isArray(match.interestsInfo.shows) &&
              match.interestsInfo.shows.length > 0 ? (
                <div>
                  <p className="card-text match-interests">Shows: </p>
                  {match.interestsInfo.shows.map((show, index) => (
                    <p
                      key={index}
                      className="card-text match-interests match-shows"
                    >
                      {show.title}
                    </p>
                  ))}
              ...
              {match.userInfo.spotify_artists ? (
                <div>
                  <p className="card-text match-interests">
                    Top Spotify Artists:{" "}
                  </p>
              ...
              <p></p>
              {Array.isArray(match.interestsInfo.hobbies) &&
              match.interestsInfo.hobbies.length > 0 ? (
                <div>
                  <p className="card-text match-interests">Hobbies: </p>
                  {match.interestsInfo.hobbies.map((hobby, index) => (
                    <p
                      key={index}
                      className="card-text match-interests match-hobbies"
                    >
                      {hobby.name}
                    </p>
                  ))}
              ...
              {match.scoreInfo.display_private ? (
                <div>
                  <p>
                    <b>{`${match.userInfo.preferredName} liked you! Contact ${match.userInfo.preferredName}:`}</b>
                  </p>
                  <p>{`Phone number: ${formatPhoneNumber(
                    match.userInfo.phoneNum
                  )}`}</p>
                  {match.userInfo.ig_username ? (
                    <a
                      className="p-link"
                      href={`https://www.instagram.com/${match.userInfo.ig_username}`}
                      target="_blank"
                    >{`Instagram Username : ${match.userInfo.ig_username}`}</a>
                  ) : null}
                </div>
              ) : null}
              {
                <button
                  ...
                  onClick={async () => {
                    ...
                    await getMatchesForUser(100, 0, true);
                    await goToLiked();
                    setIsFetching(false);
                  }}
                >
      ...
      {userMatches.length >= matchLimit && seeMoreMatches ? (
        <button
          ...
          onClick={() => {
            ...
            getMatchesForUser(matchLimit, newOffset, true);
            setOffset(newOffset);
          }}
        >
          See More
