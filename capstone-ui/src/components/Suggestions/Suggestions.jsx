...
export default function Suggestions({suggestMatch, userInfo}) {
    ...
    if(matchSuggestions.length == 0){
        let response = await fetch(DISCOVER_URL + `${latLong.lat},${latLong.long}&size=10&apikey=` + TICKETMASTER_API_KEY)
        const result = await response.json();
        ...
        <div className="suggestions">
            ...
            <div>
                ...
                {suggestions.map((suggestion, index) => (
                    ...
                    <p key={index}><a href = {suggestion.url ? suggestion.url : ""} target = "_blank" className = "p-link">{`${index + 1}. ${suggestion.name}`}</a></p>
