...
export default function Suggestions({ suggestMatch, userInfo }) {
  ...
  const getResults = async (latLong) => {
    ...
    if (userInfo?.spotify_artists && suggestMatch?.userInfo?.spotify_artists) {
      for (let i = 0; i < suggestMatch.userInfo.spotify_artists.length; i++) {
        ...
        if (contains) {
          await fetch(
            DISCOVER_URL +
              `latlong=${latLong.lat},${latLong.long}&size=2&keyword=${artist.name}&apikey=` +
              TICKETMASTER_API_KEY
          )
            .then(async (response) => {
              const result = await response.json();
              if (result._embedded?.events) {
                matchSuggestions = matchSuggestions.concat(
    ...
    if (
      matchSuggestions.length < NUM_SUGGESTIONS &&
      userInfo.interests?.hobbies &&
      suggestMatch?.interestsInfo?.hobbies
    ) {
      for (let i = 0; i < suggestMatch.interestsInfo.hobbies.length; i++) {
        ...
        if (contains) {
          await fetch(
            DISCOVER_URL +
              `latlong=${latLong.lat},${latLong.long}&size=2&keyword=${hobby.name}&apikey=` +
              TICKETMASTER_API_KEY
          )
            .then(async (response) => {
              const result = await response.json();
    ...
    if (matchSuggestions.length < NUM_SUGGESTIONS) {
      ...
      let university = userInfo.university.replace(
        "University of California-",
        "UC"
      );
      let response = await fetch(
        DISCOVER_URL +
          `latlong=${latLong.lat},${latLong.long}&size=${SUGGESTIONS_SIZE}&keyword=${university}&apikey=` +
          TICKETMASTER_API_KEY
      );
      ...
      try {
        ...
        response = await fetch(
          DISCOVER_URL +
            `latlong=${latLong.lat},${latLong.long}&size=${SUGGESTIONS_SIZE}&apikey=` +
            TICKETMASTER_API_KEY
        );
            ...
            <div key={index} className="event-card">
              <p>
                <a
                  ...
                  target="_blank"
                  className="p-link suggestion-title"
                >{`${index + 1}. ${suggestion.name}`}</a>
              </p>
